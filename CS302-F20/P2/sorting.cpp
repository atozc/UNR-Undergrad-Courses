...
void SortAlg::quickSort(std::vector<int> dataSet, int first, int last) {
    ...
    if((last - first + 1) < 4) {
        ...
        int pivotIndex = this->separate(dataSample, first, last, numOfComparisons, numOfSwaps);
...
void SortAlg::quickSort(std::vector<int>& dataSet, int first, int last, int &numOfComparisons, double &numOfSwaps) {
    if((last - first + 1) < 4) {
        ...
        int pivotIndex = this->separate(dataSet, first, last, numOfComparisons, numOfSwaps);
...
int SortAlg::separate(std::vector<int>& dataSet, int first, int last, int &numOfComparisons, double &numOfSwaps) {
...
void SortAlg::avg(int numOfRuns) {
    ...
    std::cout << "Average CPU runtime: " << avgTime << std::endl;
